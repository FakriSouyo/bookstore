...
function Card({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="card"
      className={cn('flex flex-col gap-2 border border-border bg-card py-4 text-[13px] text-card-foreground', className)}
      {...props}
    />
  );
}
...
function CardAction({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="card-action" className={cn('col-start-last row-start-1 row-span-3 self-start', className)} {...props} />;
}
